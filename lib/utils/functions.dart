...
Future<void> launchApp(String _youtubeUrl) async {
  if (await canLaunchUrl(Uri(host: _youtubeUrl))) {
    final bool _nativeAppLaunchSucceeded = await launchUrl(
      Uri(host: _youtubeUrl),
    );
    if (!_nativeAppLaunchSucceeded) {
      try {
        await launchUrl(Uri(host: _youtubeUrl));
      } catch (e) {}
    }
  }
}
...
Future<void> launch_site(String _url) async {
  if (await canLaunchUrl(Uri(host: _url))) {
    try {
      await launchUrl(Uri(host: _url));
    } catch (e) {}
  }
